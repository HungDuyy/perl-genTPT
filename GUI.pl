...
my $main = MainWindow->new(
           -title => 'TPT Test Case Generator'.$VERSION,
		   -background => 'white',
		   );
...
$main->Label(-font => 'ansi 8 bold',
		     -text => 'Select Interface list(DOCMISC) ',
		     -background => 'white',
	         )->grid(-row => 1, -column => 0, -columnspan => 2, -sticky => 'w', );

my $selDOCMISC = $main -> Entry(-width=>50) -> grid(-row=>2,-column=>1);
my $ButRefDOCMISC = $main -> Button(-width=>8, -text => ". . .", 
                           -font=>"ansi 8 bold",
		                   -command =>\&ButRefDOCMISC_push) -> grid(-row=>2,-column=>2);
						   
$main->Label(-font => 'ansi 8 bold',
		     -text => 'Select TPT Template ',
		     -background => 'white',
	)->grid(-row => 3, -column => 0, -columnspan => 2, -sticky => 'w', );

my $selARXML = $main->Entry(-width=>50) -> grid(-row=>4,-column=>1);
my $ButRefARXML = $main->Button(-width=>8, -text => ". . .", 
                        -font=>"ansi 8 bold",
						-command =>\&ButRefARXML_push) -> grid(-row=>4,-column=>2);
		
my $btnGenerateFile = $main -> Button(-text => "Create TPT Test Case!",
                                      -state=>'disabled', 
									  -font=>"ansi 8 bold",
		                              -command =>\&ButRefStart_push) -> grid(-row=>6,-column=>1);
		
#Enable Generate Button when the file is selected				
$main->repeat(500 => \&updateButtonState);

sub updateButtonState {
  my $buttonState = $btnGenerateFile->cget('-state') ;
  if (($docmisc ne "") and ($moduleARXML ne "") and ($buttonState eq 'disabled')) {
    $btnGenerateFile->configure( -state => 'normal' )
  }; 
}
...
# Functions
#===================================
sub ButRefDOCMISC_push {
  $docmisc = $main->getOpenFile(-initialdir => $start_dir,                            
                              -title => 'Select Interface list(DOCMISC)',
							  -filetypes =>[['XLS Files',  '.xls'],['All Files',  '*']]);
  $selDOCMISC->delete(0,'end');
  $selDOCMISC->insert('end',$docmisc);
  if (!defined $docmisc) { $docmisc = ''; }	
}

sub ButRefARXML_push {
  $moduleARXML = $main->getOpenFile(-initialdir => $start_dir,                            
                              -title => 'Select TPT Test Template',
							  -filetypes =>[['TPT FIles',  '.tpt'],['All Files',  '*']]);
							  
  $selARXML->delete(0,'end');
  $selARXML->insert('end',$moduleARXML);
  if (!defined $moduleARXML) { $moduleARXML = ''; }	
}
...
sub ButRefStart_push {
  ...
  #Create a log file
  my $NameFileLog = 'GenerateTestCaseTPT_GUI.log';
  ...
  readDOCMISC($docmisc);  
  if(createTestCase($moduleARXML))
  {
    $main -> messageBox(-type=>"ok", -message=>"Test Case Created");
	  exit;
  }
  else
  {
    $main -> messageBox(-type=>"ok", -message=>"Error while reading ARXML File");
	  exit;
  }	
...
sub getTptType
{
  my($docmiscType) = @_;   
  my $datatype="";
  if($docmiscType =~ m/Boolean/) { $datatype = "B_TRUE";}
...
sub createInitTestCase
{
  ...
  my $outputMinNew = calculateNewMin($inputMin, $outputMin);	
    ...
    $inputMin=~ s/\.\d+$//; 
    $inputMax=~ s/\.\d+$//;	
	#$offsetIn = int($offsetIn);
	#$offsetOut = int($offsetOut);
		$outputMinNew=~ s/\.\d+$//;
		$randomValue[0]=~ s/\.\d+$//;
		$randomValue[1]=~ s/\.\d+$//;
		$randomValue[2]=~ s/\.\d+$//;
